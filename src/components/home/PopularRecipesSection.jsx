...
async function getPopularRecipes() {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/api/recipes/popular`,
      { cache: "no-store" },
    );
    if (!res.ok) return [];
    return res.json();
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          {popularRecipes.map((recipe, index) => (
            <article
              key={recipe._id}
              className="group overflow-hidden rounded-3xl border border-white/20 bg-white/60 shadow-[0_10px_50px_rgba(244,63,94,0.06)] backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_60px_rgba(244,63,94,0.12)] dark:border-white/10 dark:bg-white/5"
            >
